...
const Notifications = ({ onItemClick }: NotificationsProps) => {
  ...
  const notificationsQuery = useNotificationsList({ tab: "unread", limit: 5 });
      ...
      {notificationsQuery.isLoading ? (
        ...
        <div className="flex items-start gap-3 max-[350px]:gap-2.5">
          ...
          <div className="min-w-0">
            <p className="text-sm font-semibold text-[var(--foreground)] max-[350px]:text-xs">
              {t("root.notifications.errorTitle")}
            </p>
            <p className="mt-1 text-sm text-[var(--muted-foreground)] max-[350px]:text-xs">
              {t("root.notifications.errorDescription")}
            </p>
          </div>
        </div>
